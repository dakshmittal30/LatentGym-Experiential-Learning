...
Everything renders inside `index.html`, which holds each top-level section as a `.panel`
div; only one panel is `.active` at a time and navigation is client-side (no routing
library). The four panels are **Home** (`#overview`), **Framework** (`#framework`, with
`fw-*` subsections: Environment Design / Evaluation / Training), **Trajectory Explorer**
(`#explorer`), and **Blog** (`#blog` — the writeup: three numbered Findings + Cite).
Navigation is a top bar (`#top-nav`) that collapses into a slide-in drawer below 760px
(toggled by `#nav-toggle`, dimmed by `#nav-backdrop`).

`index2.html` is an **alternate-layout variant** of the same content (`<body class="nav-sidebar">`,
a left sidebar nav instead of the top bar). It shares `style.css` / `script.js` and must be
kept content-synced with `index.html`; only one of the two ships as the live `index.html`.

Only two scripts are loaded — `script.js` and `explorer.js`.

- **`assets/js/script.js`** — owns panel switching (`showPanel`), the top-nav + mobile
  drawer (`setDrawer`), theme toggle (persisted in `localStorage` as `cta-theme`), hero/Fig-1
  animations (incl. `fitFig`, the zoom-to-fit that is **disabled ≤760px** so the figure
  stacks full-width on phones rather than being shrunk to a thumbnail), the interactive
  metrics simulator, and the static result tables + seven-environment cards (the `ENVS`
  array, rendered into `#env-list`). Cross-panel navigation uses
  `data-jump` / `data-anchor` / `data-prefill` attributes on elements; jumping into the
  explorer dispatches an `explorer:prefill` event that `explorer.js` listens for.
...
- **Section boundaries are hard to read.** Within a panel — the **Framework** page most of
  all, but the others too — it's not visually obvious where one section/subsection ends and
  the next begins. Idea: differentiate section backgrounds (e.g. alternate a subtle tint
  between adjacent section cards / `fw-*` subsections) so the transitions read clearly.
- **Agent-switch ("hand-off") plots — color transition is ambiguous.** In the switch plots,
  a line changes color at the hand-off point (the orange line continues *as* blue), but it
  reads as if the orange line just stops — it's unclear that orange is *becoming* blue. Make
  the transition explicit: e.g. a marker / vertical guide at the switch point, a clearer
  legend, or a labeled color change. (Static figures live in `assets/img/`, regenerated by
  `scripts/export_figures.py`; the same ambiguity may apply to the interactive hand-off
  diagram's colored tracks.)
- **Mobile (low priority):** check the exploration/exploitation efficiency formulas at the
  bottom of the hand-off diagram aren't cramped on very narrow (≤360px) screens.
